...
<header class="tw:sticky tw:top-0 tw:z-[1040] tw:backdrop-blur-xl tw:bg-[#0c092f]/80 tw:border-b tw:border-white/10" role="banner">
	<div class="tw:container tw:mx-auto tw:px-4 tw:flex tw:items-center tw:justify-between tw:h-[4.5rem]">
		<a class="tw:inline-flex tw:items-center tw:gap-3  tw:no-underline tw:font-extrabold tw:tracking-tight" href="<?php echo esc_url( home_url( '/' ) ); ?>" aria-label="DataMaq, inicio">
		...
		<nav class="tw:hidden lg:tw:flex tw:items-center tw:gap-8" aria-label="Navegación de retorno">
			<?php foreach ( $contact_page['introLinks'] as $contact_link ) : ?>
			<a href="<?php echo esc_url( $contact_link['href'] ); ?>" class="/70 tw:no-underline tw:text-sm tw:font-medium hover:tw:text-[#ff9a4d] tw:transition-colors">
				<?php echo esc_html( $contact_link['label'] ); ?>
			</a>
			<?php endforeach; ?>
		</nav>

		<div class="tw:flex tw:items-center tw:gap-4">
			<a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="tw:btn-outline tw:px-6  tw:text-sm lg:tw:hidden">
				Inicio
			</a>
